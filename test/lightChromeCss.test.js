...
describe('light chrome CSS', () => {
  ...
  it('routes topbar, dialog, storage panel, and preview backdrop through semantic chrome tokens', () => {
    ...
    assert.match(css, /#codex-media-library-root \.cml-preview__info-section--editable \{[\s\S]*background: transparent !important;/);
  ...
  it('feathers the film detail backdrop into the page instead of framing it as a rectangle', () => {
    ...
    assert.match(css, /#codex-media-library-root \.cml-film-detail-page__backdrop::before \{[\s\S]*filter: blur\(20px\);/);
    assert.match(css, /#codex-media-library-root \.cml-film-detail-page__backdrop-image \{[\s\S]*width: 110%;[\s\S]*height: 110%;[\s\S]*blur\(0\.15px\);/);
    assert.match(css, /#codex-media-library-root \.cml-film-detail-page__backdrop-image \{[\s\S]*-webkit-mask-image:[\s\S]*linear-gradient\(\s*to right,[\s\S]*transparent 0%[\s\S]*rgba\(0, 0, 0, 0\.06\) 10%[\s\S]*#000 76%/);
  });
});
