...
| --- | --- | --- |
| 2026-04-07 17:05 | Claude Code | Removed the remaining rounded-corner regression from the main photo tiles by forcing the last-applied media-tile/image/video/placeholder radius to `0`, and aligned the controllable UI fonts back to `Times New Roman` in both the overlay shell and home-route override layer. Also bumped `ui-overrides.css` to `v=4` and `media-library.css` to `v=18` for cache busting. |
