...
- latest-validation: Node syntax checks for changed JS modules; `git diff --check`; focused `previewActions.test.js` + `movieRepository.test.js` + `moviesRoute.test.js` 138 passing; latest focused metadata/search regression 126 passing; full Mocha 399 passing with Node 22; headless Chrome geometry QA confirmed backdrop pan enable/disable semantics.
