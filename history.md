...
- current-focus: Sitewide responsiveness pass is reducing full-route renders and scroll work on hot UI paths; Music playback/queue, global search, Films/Moments view-models, closed preview/album overlays, Bin grid, Moments picker, and Photos timeline lookups now prefer local/lightweight paths while Photos keeps the no-placeholder scroll regression fix.
- latest-state: 2026-May-19 (preview overlay lazy path); closed full renders no longer build preview overlay models or album-entry lists: `renderPreviewModalForViewModel(viewModel)` returns empty markup when `state.previewId` is absent, opened previews reuse view-model preview fields, and preview album entries are only built while the preview album drawer is open. Moments/Films/Search builder splits, cached scrubber/timeline refs, Photos no-placeholder scroll regression fix, and `?cmlPerf=1` diagnostics remain in place. Cache `entry-loader.js?v=5`, `app.js?v=319`, `components.js?v=109`, `moments-state.js?v=3`, `media-cache-merge.js?v=2`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=276`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-19 (preview overlay lazy path); red/green focused overlay source-contract test; Node 22 syntax checks for `app.js` and `previewActions.test.js`; full `previewActions.test.js` 133 passing with 1 pending; full Mocha under Node 22 522 passing with 1 pending; `git diff --check` clean. Default Node 24 still fails full Mocha on local `better-sqlite3` NODE_MODULE_VERSION mismatch, so use Node 22 for full-suite verification until the addon is rebuilt. Browser/live-app manual QA not run in this pass.
