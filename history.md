...
- latest-state: 2026-May-19 (scroll scrubber cache pass); `updateScrubberThumb()` now uses cached scrubber/timeline refs plus a state signature guard so scroll ticks no longer rescan all scrubber ticks and timeline sections every frame. Photos keeps the no-placeholder scroll regression fix, route-specific builders remain in place, and backend list timing diagnostics stay wired through `?cmlPerf=1`. Cache `entry-loader.js?v=5`, `app.js?v=315`, `components.js?v=109`, `moments-state.js?v=3`, `media-cache-merge.js?v=2`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=276`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-19 (scroll scrubber cache pass); Node 22 syntax checks for `app.js` and `previewActions.test.js`; focused scrubber source-contract test 1 passing; broader perf/search/resize/bin/moments/timeline/music/scrubber/backend-list grep 32 passing; full Mocha 518 passing with 1 pending; `git diff --check` clean except CRLF warnings. Browser/live-app manual QA not run in this pass.
