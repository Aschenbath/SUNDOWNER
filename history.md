...
- latest-state: 2026-May-12 13:49; `app.js?v=270`, `components.js?v=98`, `films-components.js?v=69`, `films-data.js?v=7`, `media-library.css?v=250`.
- latest-validation: Node syntax checks for changed JS modules; `git diff --check`; focused `previewActions.test.js` + `movieRepository.test.js` + `moviesRoute.test.js` 138 passing; full Mocha 397 passing with Node 22; headless Chrome geometry QA confirmed backdrop pan enable/disable semantics.
