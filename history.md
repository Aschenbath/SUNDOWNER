...
- current-focus: Sitewide responsiveness pass is reducing full-route renders on hot UI paths; Music playback/queue, global search, Bin grid, and Moments picker now prefer local DOM patches while Photos keeps the no-placeholder scroll regression fix.
- latest-state: 2026-May-18 (music scroll perf rebased Task 6); Moments picker open/close patches `[data-moments-picker-root]` locally with full-render fallback. Music queue rail remains capped at 12 visible rows with local row/queue patching; Photos keeps the prior scroll regression fix: no media-row `content-visibility` placeholders and no deferred dimension-patch render. Cache `entry-loader.js?v=5`, `app.js?v=311`, `components.js?v=109`, `moments-state.js?v=3`, `media-cache-merge.js?v=2`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=276`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-18 (music scroll perf rebased Task 6); Node 22 syntax checks for `app.js`, `components.js`, and `momentsAppState.test.js`; full `momentsAppState.test.js` 23 passing; focused sitewide perf/search/resize/bin grep in `previewActions.test.js` 19 passing; `git diff --check` clean except CRLF warnings. Browser/live-app manual QA not run in this pass.
