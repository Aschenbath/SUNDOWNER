...
- current-focus: Photos scroll responsiveness via content-visibility + idle dimension patch; preserve the Moments save-UX baseline and the prior Photos D1/KV hybrid supplement, provider-identity dedupe, recycle-bin alias blocking, metadata fallback, timestamp sorting, absolute pagination, and legacy/generic `FileType` extension inference.
- latest-state: 2026-May-18 (scroll pass); Photos timeline now stays mounted under 720 items and relies on `content-visibility: auto` + per-row `contain-intrinsic-size`; `TIMELINE_DIMENSION_PATCH_IDLE_MS` (600ms) defers `applyDimensionPatch` while the user is mid-scroll so image-load dimension corrections no longer trigger a full `render()`. Cache `entry-loader.js?v=5`, `app.js?v=308`, `components.js?v=107`, `moments-state.js?v=3`, `media-cache-merge.js?v=2`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=274`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-18 (scroll pass); Node 22 focused `previewActions.test.js --grep "normal Photos timelines"` 1 passing; full `previewActions.test.js` 116 passing with 1 pending; full Mocha 504 passing with 1 pending; pushed `8bee0cd` to `origin/main`. Browser/live-app manual QA not run in this pass.
