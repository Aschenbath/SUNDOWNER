...
- latest-validation: Node syntax checks for changed JS modules; `git diff --check`; focused `previewActions.test.js` 100 passing; focused `movieRepository.test.js` + `moviesRoute.test.js` 40 passing; full Mocha 399 passing with Node 22; headless Chrome geometry QA from the previous pass confirmed backdrop pan enable/disable semantics.
