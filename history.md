...
- latest-state: 2026-May-19 (Photos active-section offset cache); Photos scroll now caches timeline section `offsetTop` values when scrubber/timeline refs populate, so active section/year detection binary-searches `refs.timelineSectionOffsetTops` without scroll-time `.offsetTop` layout reads, refreshes the cache after virtual timeline DOM patches, and skips active-section updates when the offset cache is incomplete. Earlier Photos virtual-window reuse, closed preview overlay lazy path, Moments/Films/Search builder splits, cached scrubber/timeline refs, Photos no-placeholder scroll regression fix, and `?cmlPerf=1` diagnostics remain in place. Cache `entry-loader.js?v=5`, `app.js?v=323`, `components.js?v=109`, `moments-state.js?v=3`, `media-cache-merge.js?v=2`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=276`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-19 (Photos active-section offset cache); Node 22 syntax checks for `app.js` and `previewActions.test.js`; focused scroll grep 4 passing; full `previewActions.test.js` 134 passing with 1 pending; full Mocha under Node 22 523 passing with 1 pending; `git diff --check` clean except CRLF warnings. Default Node 24 still fails full Mocha on local `better-sqlite3` NODE_MODULE_VERSION mismatch, so use Node 22 for full-suite verification until the addon is rebuilt. Browser/live-app manual QA not run in this pass.
