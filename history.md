...
- latest-state: 2026-May-18 (sitewide responsiveness pass); render/action diagnostics stay gated behind `?cmlPerf=1`; Music playback/queue, global search, Bin grid, Moments picker, resize, Photos timeline lookup, and route-specific view-model construction now use lighter local paths where possible. `/api/manage/list` now returns `listTiming` for D1 and legacy index responses, and `fetchListPage()` records backend list timing rows through the perf diagnostic gate. Photos keeps the no-placeholder scroll regression fix: no media-row `content-visibility` placeholders and no deferred dimension-patch render. Cache `entry-loader.js?v=5`, `app.js?v=314`, `components.js?v=109`, `moments-state.js?v=3`, `media-cache-merge.js?v=2`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=276`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-18 (sitewide responsiveness pass); Node 22 syntax checks for `app.js`, `components.js`, `functions/api/manage/list.js`, `test/d1Metadata.test.js`, and `test/previewActions.test.js`; focused D1 list-route grep 8 passing; focused sitewide perf/search/resize/bin/moments/timeline/music/backend-list grep 31 passing; full Mocha 517 passing with 1 pending; `git diff --check` clean except CRLF warnings. Browser/live-app manual QA not run in this pass.
