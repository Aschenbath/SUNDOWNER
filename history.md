...
- current-focus: Music dashboard scroll lock is fixed at the internal scroll container; global shell remains untouched.
- latest-state: 2026-May-15 15:34; Music dashboard scroll cache `entry-loader.js?v=5`, `app.js?v=292`, `components.js?v=100`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=266`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-15 15:34; red-first scroll regression failed on missing `grid-template-rows: minmax(0, 1fr)` / `min-height: 0`, then focused Music+scroll `previewActions.test.js` passed 7/7 and full Mocha passed 418/1 pending.
