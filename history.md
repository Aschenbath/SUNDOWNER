...
- current-focus: Sitewide responsiveness pass is reducing full-route renders and scroll work on hot UI paths; Music playback/queue, global search, Films/Moments view-models, Bin grid, Moments picker, and Photos timeline lookups now prefer local/lightweight paths while Photos keeps the no-placeholder scroll regression fix.
- latest-state: 2026-May-19 (Moments builder split); Moments no longer proxies `getLegacyViewModel()`: `getMomentsViewModel()` now builds only current audio/queue state, Moments attachment preview fields, and selection action flags through `buildViewModelResult()`, leaving Photos timeline/search/video album/Films calculations off the Moments route path. Films/Search builder splits, cached scrubber/timeline refs, Photos no-placeholder scroll regression fix, and `?cmlPerf=1` diagnostics remain in place. Cache `entry-loader.js?v=5`, `app.js?v=318`, `components.js?v=109`, `moments-state.js?v=3`, `media-cache-merge.js?v=2`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=276`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-19 (Moments builder split); red/green focused Moments source-contract test; Node 22 syntax check for `app.js`; full `previewActions.test.js` 132 passing with 1 pending; full Mocha under Node 22 521 passing with 1 pending; `git diff --check` clean except CRLF warnings. Default Node 24 still fails full Mocha on local `better-sqlite3` NODE_MODULE_VERSION mismatch, so use Node 22 for full-suite verification until the addon is rebuilt. Browser/live-app manual QA not run in this pass.
