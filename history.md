...
- latest-state: 2026-May-18 (music scroll perf rebased Task 7); Photos virtual timeline row ranges and active-year updates now use binary lookup helpers. Moments picker open/close patches `[data-moments-picker-root]` locally; Music queue rail remains capped at 12 visible rows with local row/queue patching; Photos keeps the prior scroll regression fix: no media-row `content-visibility` placeholders and no deferred dimension-patch render. Cache `entry-loader.js?v=5`, `app.js?v=312`, `components.js?v=109`, `moments-state.js?v=3`, `media-cache-merge.js?v=2`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=276`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-18 (music scroll perf rebased Task 7); Node 22 syntax checks for `app.js` and `previewActions.test.js`; focused timeline tests 2 passing; broader performance grep in `previewActions.test.js` 30 passing; `git diff --check` clean except CRLF warnings. Browser/live-app manual QA not run in this pass.
