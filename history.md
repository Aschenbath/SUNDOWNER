...
- current-focus: Music dashboard layout and scroll are fixed; legacy CSS overrides removed; global shell remains untouched.
- latest-state: 2026-May-15 16:12; Music layout fix cache `entry-loader.js?v=5`, `app.js?v=292`, `components.js?v=100`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=267`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-15 16:12; red-first scroll regression failed on missing `grid-template-rows: minmax(0, 1fr)` / `min-height: 0`, then focused Music+scroll `previewActions.test.js` passed 7/7 and full Mocha passed 418/1 pending.
