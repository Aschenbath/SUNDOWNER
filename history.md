...
- current-focus: Photos library recovery after Moments uploads; media listing must preserve D1/KV hybrid recovery, metadata fallback, timestamp sorting, absolute start/count pagination, and legacy/generic `FileType` extension inference.
...
- latest-validation: 2026-May-17; syntax checks for `manage/list.js`, `app.js`, `d1Metadata.test.js`, and `momentsAppState.test.js`; full Mocha 486 passing with 1 pending; `git diff --check` clean except CRLF warnings.
