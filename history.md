...
- latest-state: 2026-May-18 (music scroll perf rebased Task 8); `getViewModel()` now dispatches through route-specific builders backed by a shared base context/result shape. Music uses a lighter builder that avoids Photos timeline/search/video/collection work; Photos/Search/Films/Moments currently retain legacy builder parity for safer staged extraction. Moments picker open/close patches `[data-moments-picker-root]` locally; Music queue rail remains capped at 12 visible rows with local row/queue patching; Photos keeps the prior scroll regression fix: no media-row `content-visibility` placeholders and no deferred dimension-patch render. Cache `entry-loader.js?v=5`, `app.js?v=313`, `components.js?v=109`, `moments-state.js?v=3`, `media-cache-merge.js?v=2`, `films-components.js?v=81`, `films-data.js?v=7`, `media-library.css?v=276`; SUNDOWNER local dev/test default port remains `8787`, not `8080`.
- latest-validation: 2026-May-18 (music scroll perf rebased Task 8); Node 22 syntax checks for `app.js` and `previewActions.test.js`; focused view-model red/green test; focused Music/search/timeline/view-model grep 12 passing; broader performance grep 30 passing; full Mocha 516 passing with 1 pending; `git diff --check` clean except CRLF warnings. Browser/live-app manual QA not run in this pass.
